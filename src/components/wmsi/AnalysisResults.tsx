...
export default function AnalysisResults({ results, tab, setTab, reset, savedToDb, userEmail }: Props) {
  ...
  // SAFE ACCESS HELPERS (Mencegah crash map)
  const safeKeywords = Array.isArray(seo?.content_semantic_analysis?.keywords) ? seo.content_semantic_analysis.keywords : [];
  const safeStrengths = Array.isArray(vision?.key_strengths) ? vision.key_strengths : [];
  const safeImprovements = Array.isArray(vision?.critical_improvements) ? vision.critical_improvements : [];
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div id="screen-dashboard">
        ...
        <div style={{ background: '#fff', borderRadius: 16, padding: 24, boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)', minHeight: 300 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 24 }}>
                <InfoCard title="Key Strengths" color="#10b981">
                  <ul style={{ paddingLeft: 16, margin: 0 }}>
                    {safeStrengths.length > 0 ? safeStrengths.map((s: string, i: number) => <li key={i} style={{ marginBottom: 4 }}>{s}</li>) : <li>Data tidak tersedia.</li>}
                  </ul>
                </InfoCard>
                <InfoCard title="Critical Improvements" color="#ef4444">
                  <ul style={{ paddingLeft: 16, margin: 0 }}>
                    {safeImprovements.length > 0 ? safeImprovements.map((s: string, i: number) => <li key={i} style={{ marginBottom: 4 }}>{s}</li>) : <li>Tidak ada isu kritikal.</li>}
                  </ul>
                </InfoCard>
              </div>
            </div>
            ...
            <div>
              ...
              <div style={{ display: 'grid', gap: 16 }}>
                <div style={{ background: '#f8fafc', padding: 16, borderRadius: 12 }}>
                  ...
                  {seo?.technical_audit ? (<div style={{ display: 'grid', gap: 8 }}><ListItem label="Core Web Vitals" value={seo.technical_audit.core_web_vitals_assessment || 'N/A'} /><ListItem label="Mobile Friendly" value={seo.technical_audit.mobile_friendliness || 'N/A'} /><ListItem label="SSL & Security" value={seo.technical_audit.ssl_security || 'N/A'} /></div>) : <p style={{fontSize:12}}>Data tidak tersedia.</p>}
                </div>
                <div style={{ background: '#f8fafc', padding: 16, borderRadius: 12 }}>
                  ...
                  {/* SAFE MAP FOR KEYWORDS */}
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                    {safeKeywords.length > 0 ? safeKeywords.map((k: any, i: number) => (
                      <span key={i} style={{ background: '#e0f2fe', color: '#0284c7', padding: '4px 10px', borderRadius: 20, fontSize: 11, fontWeight: 600 }}>
                        {k.keyword}
                      </span>
                    )) : <span style={{fontSize:11, color:'#64748b'}}>Keywords not detected.</span>}
                  </div>
                </div>
              </div>
            </div>
          ...
          {/* ... (Tab UI, Marketing, WQ tetap sama, logic aman karena pakai object access biasa) ... */}
          {tab === 'ui' && (
            <div>
              ...
              <InfoCard title="Visual Hierarchy Analysis" color="#f59e0b">{vision.ui?.visual_hierarchy?.analysis || "Analisis tidak tersedia."}</InfoCard>
            </div>
          )}
      ...
      {/* HIDDEN PRINT AREA (Tetap dipertahankan) */}
